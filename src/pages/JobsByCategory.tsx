...
export default function JobsByCategory() {
  ...
  const { data: jobCategories = [] } = useQuery<Category[]>({
  ...
  const getSelectedCategoryObjects = (): {
    ...
    let selectedSubcategory: Subcategory | undefined;
    let selectedSpecialization: Specialization | undefined;

    if (!jobCategories.length || !selectedFilters.categoryId) {
      return { selectedCategory, selectedSubcategory, selectedSpecialization };
    }

    selectedCategory = jobCategories.find(cat => cat.id === selectedFilters.categoryId);
    ...
    if (
      ...
      selectedSubcategory = selectedCategory.subcategories.find(
        sub => sub.id === selectedFilters.subcategoryId
      );

      if (
        selectedSubcategory &&
        selectedFilters.specializationId &&
        selectedSubcategory.specializations
      ) {
        selectedSpecialization = selectedSubcategory.specializations.find(
          spec => spec.id === selectedFilters.specializationId
        );
      }
    }
