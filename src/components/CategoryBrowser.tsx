...
export default function CategoryBrowser({ onCategorySelect, selectedCategory, selectedSubcategory, selectedSpecialization }: CategoryBrowserProps) {
  ...
  const [selectedSubcategoryData, setSelectedSubcategoryData] = useState<Subcategory | null>(null);
  ...
  // };
  const handleSpecializationClick = (specialization: Specialization) => {
    if (!selectedCategoryData || !selectedSubcategoryData) return;
                ...
                <CardContent className="pt-0">
                  <div className="space-y-3">
                    <div className="flex flex-wrap gap-1">
                      {specialization.skills?.slice(0, 4).map((skill: string, index: number) => (
                      ...
                      {specialization.skills && specialization.skills.length > 4 && (
                        <Badge variant="outline" className="text-xs">
                          +{specialization.skills.length - 4} more
                        </Badge>
                      )}
